...
class Pengetahuan
{
    ...
    public function prosesUbahPengetahuan()
    {
        ...
        $bobot = $input['bobot'];
         ...
         // check duplicate
         $check = $this->_db->other_query("SELECT * FROM tb_pengetahuan WHERE kode_gejala = '$gejala' AND kode_kerusakan = '$kerusakan' AND id_pengetahuan != '$input[id]'", 2);
         if ($check) {
             echo json_encode(array('status' => 0, 'msg' => 'Data sudah ada'));
             die;
         }
        ...
        // query update
        $update = $this->_db->edit("UPDATE tb_pengetahuan SET kode_gejala = '$gejala', kode_kerusakan = '$kerusakan', bobot = '$bobot' WHERE id_pengetahuan = '$input[id]'");
        if ($update) {
            $res['status'] = 1;
